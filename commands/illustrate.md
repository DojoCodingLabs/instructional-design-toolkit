...
| `<holding>/<course-slug>/*.png` | **Staging** — raw generations (good and bad takes) reviewed before promotion. Consumer-specific holding folder (dojo-academy: `nanobanana-output/`, gitignored); namespaced per course. Neutral default: `.illustrate-staging/` |
...
1. Resolve the target course dir from `$ARGUMENTS` (consumer layout;
   dojo-academy: `content/courses/<slug>/`). Optional module scope.
...
palette — e.g. dojo-academy contributes: dark navy-purple `#272749`
...
`dojo-academy` is installed: brand palette sentence, holding-folder path,
